    ...
    var body: some View {
        VStack(spacing: 20) {
            ...
            Button(action: {
                ...
                Label(
                    audioVM.isPlaying ? "Pause" : "Play",
                    systemImage: audioVM.isPlaying ? "pause.circle.fill" : "play.circle.fill"
                )
                .font(.largeTitle)
            }
            .disabled(audioVM.isLoading)
...
class AudioPlayerViewModel: ObservableObject {
    ...
    func play(urlString: String) {
        // If same URL and player exists, just resume
        if urlString == currentURL, let player = player {
            player.play()
            isPlaying = true
            return
        ...
        // Observe status with proper cleanup
        statusObserver = playerItem.observe(\.status, options: [.new]) { [weak self] item, _ in
            DispatchQueue.main.async {
                self?.handlePlayerStatus(item.status)
            }
